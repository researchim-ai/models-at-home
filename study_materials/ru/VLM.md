...
## Практика 2026: как тюнить small VLM дома

Если цель не исследование на кластере, а рабочий домашний пайплайн на 24GB или 2x3090, то сегодня базовая стратегия обычно такая:

- брать не full fine-tune, а `LoRA`/`QLoRA`;
- в качестве основных small VLM использовать `Qwen 3.5 0.8B` и `Qwen 3.5 2B`;
- для более зрелого instruct-бейзлайна держать под рукой `Qwen2.5-VL` и `LLaVA-NeXT`;
- для маленького `pretrain` делать не обучение с нуля, а **continued multimodal pretraining/alignment** на caption/OCR/VQA данных;
- по умолчанию держать включёнными `gradient checkpointing`, `BF16` и `FlashAttention 2`, а vision tower часто замораживать на первых прогонах.

Практические сценарии, которые хорошо подходят под домашнее железо:

| Сценарий | Что делать |
|---------|------------|
| `FastSFT_0.8B_24GB` | Быстрое instruction tuning на 0.8B с `QLoRA`, небольшим batch и умеренным визуальным бюджетом |
| `FastSFT_2B_2x3090` | Основной универсальный режим для 2B VLM на двух 3090 |
| `SmallPretrain_0.8B_caption_alignment` | Маленький multimodal pretrain/alignment на caption и OCR-подобных корпусах |
| `OCRTune_2B_doc_qa` | Дообучение под документы, скриншоты, таблицы и OCR-задачи |
| `ExperimentalVLM_GRPO` | Экспериментальный reward-guided режим для image-conditioned ответов |

Что особенно важно для современных Qwen-VLM:

- у моделей есть **визуальный токен-бюджет**, поэтому `min_pixels`/`max_pixels` реально влияют на память и качество;
- message-based формат данных лучше одной строки, потому что он ближе к реальному чат-шаблону модели;
- для SFT полезнее короткие, чистые, однозначные ответы, чем очень длинные размытые пояснения;
- для OCR/doc/UI задач часто важнее поднять визуальное разрешение, чем увеличивать `seq_len`.
