...
Подробный учебный материал о больших языковых моделях (Large Language Models): из чего они состоят, как работают и как их тренируют. Материал рассчитан на новичков: ключевые термины поясняются при первом появлении, в начале есть краткий словарик. Если что-то останется непонятным — можно вернуться к словарику или к разделу про архитектуру Transformer ([transformer.md](transformer.md)). Для моделей, работающих с изображениями и текстом, см. [VLM.md](VLM.md).
...
|--------|---------------------|
| **Токен** | Минимальная единица текста, с которой работает модель. Чаще всего это **подслово** (кусок слова): например «При» и «вет» — два токена вместо одного слова «Привет». Реже — целое короткое слово или один символ. |
| **Токенизатор** | Алгоритм и словарь, которые разбивают текст на токены и превращают их в числа (id). Например: «Привет» → [1045, 3921]. |
| **Словарь (vocab)** | Список всех допустимых токенов. Каждому токену сопоставлен номер (id). Размер словаря — сколько таких токенов (часто 32k–128k). |
| **id токена** | Целое число — номер токена в словаре. Модель внутри работает с числами; по id потом снова получают текст при декодировании. |
| **Промпт** | Текст, который пользователь подаёт модели «на вход» — вопрос, начало фразы, контекст. Модель по нему предсказывает продолжение. |
| **Контекст** | Текст, который модель «видит» в данный момент: промпт + уже сгенерированные токены. Длина контекста ограничена (окно контекста). |
| **Авторегрессия** | Генерация по одному токену за раз: каждый следующий токен зависит от всех предыдущих. Модель не выдаёт весь ответ сразу, а «допишет слово — подумала — следующее слово». |
| **Параметры модели** | Обучаемые числа в нейросети (веса матриц, сдвиги). «7B» (7 миллиардов) — это примерно столько параметров у модели вроде LLaMA-7B. |
| **Эмбеддинг** | Представление токена в виде вектора (набора чисел фиксированной длины). Получают по таблице: id токена → строка матрицы. |
| **OOV (out of vocabulary)** | Токен или слово, которого нет в словаре. При разбиении по словам незнакомое слово часто заменяют одним спецтокеном [UNK]. При подсловах (BPE) незнакомое слово собирают из известных кусочков. |
| **EOS (end of sequence)** | Специальный токен «конец последовательности». Модель может выдать его, чтобы сигнализировать: ответ закончен. |

---

## Что такое LLM

**LLM (Large Language Model)** — это **большая языковая модель**: нейросеть, обученная предсказывать **следующий** элемент текста. Элемент текста в таких моделях — это **токен** (чаще всего подслово или короткое слово, а не символ и не целая фраза). То есть модель всё время отвечает на вопрос: «учитывая уже написанный текст, какой токен идёт дальше с наибольшей вероятностью?» Формально это **модель условной вероятности**:

$$P(x_{t+1} \mid x_1, x_2, \ldots, x_t)$$

где $x_i$ — токены. Генерация идёт **авторегрессионно**: модель выдаёт ровно **один** следующий токен, затем этот токен добавляется к контексту, и снова предсказывается один токен — и так до конца ответа или до спецтокена остановки (например EOS).

```
  Авторегрессия: на каждом шаге модель выдаёт ровно ОДИН токен. Токен = подслово
  (кусок слова или целое короткое слово), не символ и не целая фраза.

  Текст (промпт):   "Столица Франции — "
                            │
                            ▼  токенизатор (BPE/SentencePiece) → последовательность токенов
  ┌──────────┬──────────┬──────────┬──────────┬──────────┐      ┌─────────┐
  │  токен 1 │  токен 2 │  токен 3 │  токен 4 │  токен 5 │  →   │ токен 6 │
  │  "Сто"   │  "лица"  │  " Фран" │  "ции"   │  " —"    │      │   ?     │
  └──────────┴──────────┴──────────┴──────────┴──────────┘      └─────────┘
       x₁          x₂          x₃          x₄          x₅           ↑
  ────────────────────────────────────────────────────────────────  модель
  Границы токенов не совпадают с пробелами: "Столица" = "Сто"+"лица",   предсказывает
  "Франции" = " Фран"+"ции". Один токен может быть с пробелом в начале.  один токен
                            │
                            ▼
  Шаг 1: модель выдаёт токен "Па"   →  контекст: ... " —" "Па"
  Шаг 2: модель выдаёт токен "риж"  →  контекст: ... "Па" "риж"
  Шаг 3: модель выдаёт токен "."    →  контекст: ... "риж" "."
  Шаг 4: модель выдаёт [EOS]        →  стоп.

  Итого: каждый шаг добавляет ровно один токен (подслово); итоговый текст
  собирается конкатенацией токенов: "Сто"+"лица"+" Фран"+"ции"+" —"+"Па"+"риж"+"."
```
...
Модель «видит» только последние **N** токенов — это и есть **длина контекста** (context length), или **окно контекста**. Типичные значения: 2k (2000 токенов), 4k, 8k, 32k, 128k. Буква «k» здесь значит тысячи: 8k = 8000 токенов. Всё, что было раньше этих N токенов, формально недоступно — модель не может «прочитать» более старую часть входа. На практике при обучении на длинных текстах модель всё же учится как бы сжимать важную информацию в скрытых представлениях, но явно хранить произвольно длинную историю она не может.
...
Текст перед подачей в модель превращается в последовательность целых чисел — **ид токенов** (каждый токен заменяется на его номер в словаре). Этим занимается **токенизатор**: он знает правила разбиения и словарь. От выбора токенизатора и размера словаря зависят длина последовательностей (сколько чисел получится из одной фразы), качество на разных языках и способность обрабатывать редкие и новые слова.

### Зачем не по символам и не по словам

Три подхода к единице токена:

| Подход        | Плюсы                    | Минусы                                      |
|---------------|--------------------------|---------------------------------------------|
| **По символам** | Маленький словарь, любое слово | Очень длинные последовательности, слабая модель |
| **По словам**  | Короткие последовательности   | Огромный словарь, много словоформ; незнакомые слова (OOV) превращаются в один токен «неизвестно» [UNK] |
| **Подслова (subword)** | Компромисс: словарь 10k–100k, новые слова через кусочки | Нужно обучать токенизатор и хранить словарь |

**Подсловная токенизация** разбивает текст на куски: частые слова могут быть одним токеном, редкие — несколькими подсловами. Незнакомое слово представляется как конкатенация известных подслов без потери символов.

```
  Сравнение: одно и то же предложение при разной токенизации

  Текст: "Нейросеть обучают на данных."

  По символам (каждый символ = токен):
  ┌──┬──┬──┬──┬──┬──┬──┬──┬──┬──┬──┬──┬──┬──┬──┬──┬──┬──┬──┬──┬──┬──┬──┬──┬──┬──┬──┬──┐
  │Н │е │й │р │о │с │е │т │ь │  │о │б │у │ч │а │у │т │  │н │а │  │д │а │н │н │ы │х │. │  ...
  └──┴──┴──┴──┴──┴──┴──┴──┴──┴──┴──┴──┴──┴──┴──┴──┴──┴──┴──┴──┴──┴──┴──┴──┴──┴──┴──┘
  Много токенов → дорого и длинный контекст.

  По словам (словарь 100k):
  ┌──────────┬──────────┬────────┬────┬────────┐
  │ Нейросеть│ обучают  │   на   │данных│   .   │   "Нейросеть" может быть OOV → один [UNK]
  └──────────┴──────────┴────────┴────┴────────┘

  По подсловам (BPE, словарь ~32k):
  ┌────────┬────┬────┬────┬────┬────┬────┬────┬────┬────┐
  │ Нейрос │ е  │ т  │ ь  │ об │уч │ а  │ у  │ т  │ на │ ...  (пример; реальное разбиение зависит от словаря)
  └────────┴────┴────┴────┴────┴────┴────┴────┴────┴────┘
  Баланс длины и выразительности.
```
...
Идея: начать с атомарных единиц (символы или байты) и **итеративно сливать** самую частую пару соседних токенов в один новый токен. Словарь растёт до заданного размера.

**Алгоритм обучения BPE:**

1. **Инициализация:** словарь = все символы (или все 256 байтов в byte-level BPE). Каждый символ — отдельный токен.
2. **Подсчёт пар:** по корпусу текста считаем, сколько раз каждая пара токенов идёт подряд (в текущей сегментации).
3. **Слияние:** выбираем пару с максимальной частотой, добавляем её в словарь как один новый токен, во всём корпусе заменяем эту пару на новый токен.
4. Повторяем шаги 2–3 до достижения целевого размера словаря (например 32k–50k).
5. Сохраняем **список правил слияния** (порядок важен): при кодировании применяем слияния в этом порядке.

```
  BPE: цикл обучения (общая схема)

  ┌─────────────┐     текущая сегментация корпуса (сначала по символам)
  │   Корпус    │
  └──────┬──────┘
         │
         ▼
  ┌─────────────┐     считаем частоты пар (A,B), идущих подряд
  │ Подсчёт пар │     например: ("а","н")=5000, ("н","а")=3000, ...
  └──────┬──────┘
         │
         ▼
  ┌─────────────┐     выбираем пару с макс. частотой, добавляем в словарь,
  │  Слияние    │     во всём корпусе заменяем эту пару на один токен
  │  (merge)    │
  └──────┬──────┘
         │
         ▼
  размер словаря < целевой?  ──Нет──►  стоп, сохраняем список правил слияния
         │
        Да
         │
         └──────────────────────────► повторяем подсчёт пар
```

```
  BPE: обучение на примере слова "банан" (упрощённо, только это слово в корпусе)

  Начальное состояние: разбиение по символам.
  ┌─────┬─────┬─────┬─────┬─────┐
  │  б  │  а  │  н  │  а  │  н  │
  └─────┴─────┴─────┴─────┴─────┘
  Пары: (б,а)=1, (а,н)=2, (н,а)=1. Чаще всего (а,н) → сливаем в "ан".

  После 1-го слияния (а + н → "ан"):
  ┌─────┬─────┬─────┬─────┐
  │  б  │ ан  │  а  │  н  │
  └─────┴─────┴─────┴─────┘
  Пары: (б,ан)=1, (ан,а)=1, (а,н)=1. Сливаем (ан,а) → "ана" или следующую по частоте.

  После нескольких слияний (условно):
  ┌─────┬─────────┬─────┐
  │  б  │  анан   │  н  │   или   │ ба │ нан │  (зависит от корпуса и порядка слияний)
  └─────┴─────────┴─────┘
  Итог: слово представлено меньшим числом токенов.
```

**Кодирование нового текста:** разбиваем текст на символы, затем применяем правила слияния **по порядку** (как они были добавлены при обучении). Каждое правило: если в текущей последовательности токенов есть эта пара подряд — заменяем её на один токен.

```
  BPE: кодирование (encode)

  Текст: "банан"
  Шаг 0:  [ б ] [ а ] [ н ] [ а ] [ н ]

  Применяем правила по очереди (как в обучении):
  Правило 1: (а, н) → "ан"   →  [ б ] [ ан ] [ а ] [ н ]
  Правило 2: (ан, а) → "ана" →  [ б ] [ ана ] [ н ]
  Правило 3: (б, ана) → "бана" → [ бана ] [ н ]
  ... (дальше по списку правил, пока применимо)

  Итоговая последовательность токенов → преобразуем в id по словарю.
```

**Декодирование:** по списку id восстанавливаем строки токенов (каждый id — это строка, например "бана" и "н"). Конкатенируем их — получаем исходный текст. Спецтокены (EOS, pad) не декодируются в символы.

```
  BPE: декодирование (decode)

  id последовательность:  [ 101, 204, 305 ]
        │
        ▼  по словарю:  101 → "При",  204 → "вет",  305 → ","
  Токены:  "При"  "вет"  ","
        │
        ▼  конкатенация (без пробелов между кусками, если не было спец. правил)
  Текст:   "Привет,"
```

**Byte-level BPE (как в GPT-2):** базовые единицы — **байты** (0–255). Любая строка представима как последовательность байтов. Плюс: не бывает «неизвестного символа», эмодзи и редкие Unicode разбиваются на байты. Минус: для не-ASCII текста много мелких токенов, словарь частично «тратится» на байты.

**Размер словаря — компромисс:** маленький (8k–16k) — длиннее последовательности, больше вычислений; большой (64k–128k) — короче последовательности, но больше параметров в embedding и LM head, и многие токены реже встречаются в обучении.

### SentencePiece и аналоги

**SentencePiece** — не один алгоритм, а фреймворк: текст сначала переводится в последовательность символов (или байтов), затем применяется один из методов сегментации. Важно: **пробелы тоже кодируются** (например, пробел заменяется на символ `_` и входит в токены), поэтому декодирование однозначно восстанавливает текст без потери пробелов.

- **BPE внутри SentencePiece:** та же идея слияния частых пар; часто реализация на уровне Unicode или байтов.
- **Unigram:** другой принцип — каждому токену в словаре приписывается вероятность; сегментация текста выбирается так, чтобы произведение вероятностей токенов было максимальным (или сумма логарифмов). Словарь обучают итеративно: переоценка вероятностей токенов и удаление маловероятных / добавление кандидатов. Часто даёт более ровное качество по разным языкам.
- В современных LLM часто используют SentencePiece (Llama, Mistral) или BPE (GPT-2; LLaMA тоже использует BPE-style в SentencePiece).

```
  Unigram (схема): сегментация по максимуму вероятности

  Слово: "обучение"
  Возможные разбиения (словарь содержит подслова и им приписаны вероятности):

  Вариант A:  [ об ] [ у ] [ ч ] [ е ] [ ни ] [ е ]   →  log P(об)+log P(у)+...
  Вариант B:  [ обуч ] [ е ] [ ни ] [ е ]             →  log P(обуч)+...
  Вариант C:  [ обучение ]                             →  log P(обучение)

  Выбирается разбиение с максимальной суммой log P(токен). Словарь и вероятности
  обучаются на корпусе (EM-подобная процедура).
```

```
  Общий пайплайн токенизации (SentencePiece-стиль)

  "Привет, мир!"          предобработка (нормализация пробелов и т.д.)
        │
        ▼
  ┌─────────────────────────────────────────────────────────┐
  │  Опционально: pre-tokenization (разбить по пробелам,    │
  │  пунктуации) → "Привет" "," "мир" "!"                   │
  └─────────────────────────────────────────────────────────┘
        │
        ▼  применение BPE / Unigram по правилам словаря
  ┌─────┬─────┬─────┬─────┬─────┐
  │При  │вет  │  ,  │ мир │  !  │   (пример разбиения)
  └─────┴─────┴─────┴─────┴─────┘
        │
        ▼  lookup в словаре: строка → id
  [ 1045, 3921, 11, 482, 0 ]
```
...
- **BOS / EOS** (beginning/end of sequence) — границы последовательности; генерация часто останавливается при выдаче EOS.
- **Pad** — выравнивание до одной длины в батче при обучении; при расчёте loss позиции с pad обычно маскируют.
- В чат-моделях: `<|im_start|>`, `<|im_end|>`, `<|user|>`, `<|assistant|>` и т.п. для разметки ролей в диалоге.

Токенизатор обычно **обучают** на репрезентативной выборке текста до обучения модели, затем замораживают. От выбора токенизатора и размера словаря зависят длина последовательности и качество на разных языках.

```
  Токенизация: итоговый вид (текст → id)
...
Модель внутри работает не с целыми числами (id токенов), а с **векторами** — наборами чисел фиксированной длины. Эта длина называется **hidden_size** (типично 768, 1024, 2048, 4096, 8192). Первый шаг в модели — **превратить каждый id токена в вектор** такой размерности. Этот шаг называют **эмбеддингом** (embedding): мы «погружаем» дискретный токен в непрерывное векторное пространство, с которым удобно работать нейросети.

### Token embedding

- По сути это **таблица (матрица)** размером **vocab_size × hidden_size**: строк столько, сколько токенов в словаре, в каждой строке — вектор длины hidden_size.
- Токен с id $i$ кодируется как **$i$-я строка** этой матрицы. Такую операцию называют **lookup** (поиск по индексу): по номеру достаём готовый вектор.
- Эти веса (вся матрица) **обучаются** вместе с остальной сетью — то есть векторы токенов подстраиваются под задачу в процессе обучения.
...
Сердце LLM — **повторяющийся блок** (слой Transformer). Такой блок много раз подряд составляет «тело» модели. Внутри одного блока два основных подуровня: **Self-Attention** (каждая позиция смотрит на другие и обновляет своё представление) и **Feed-Forward Network (FFN)** (нелинейное преобразование по каждой позиции отдельно). Между ними и вокруг — **нормализация** и **остаточные связи**, чтобы обучение было стабильным. Подробнее про устройство блоков и внимания см. в [transformer.md](transformer.md).

### Self-Attention (самовнимание)

Модель для **каждой** позиции в последовательности «смотрит» на все **доступные** ей токены (в LLM при генерации — только на себя и на предыдущие, не на будущие) и формирует новый вектор как **взвешенную сумму** их представлений. Веса «на кого смотреть» модель выучивает сама.
